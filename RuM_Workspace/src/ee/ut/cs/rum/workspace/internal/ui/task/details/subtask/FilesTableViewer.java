...
public class FilesTableViewer extends TableViewer implements RumUpdatableView {
	...
	private TaskSubTaskInfoLeft taskSubTaskInfoLeft;
	private List<UserFile> userFiles;

	public FilesTableViewer(TaskSubTaskInfoLeft taskSubTaskInfoLeft, RumController rumController) {
		super(taskSubTaskInfoLeft, SWT.H_SCROLL | SWT.V_SCROLL | SWT.BORDER);

		this.display=Display.getCurrent();
		rumController.registerView(this, ControllerEntityType.USER_FILE);

		this.taskSubTaskInfoLeft=taskSubTaskInfoLeft;
		...
		userFiles = UserFileAccess.getSubTaskUserFilesDataFromDb(taskSubTaskInfoLeft.getSubTask().getId());
		this.setInput(userFiles);
	}

	private void createColumns(FilesTableViewer viewer) {
		String[] titles = { "Name", "Modified at", "Download"};
		int[] bounds = { 200, 125, 80 };
		...
		TableViewerColumn lastModifiedAtColumn = createTableViewerColumn(titles[1], bounds[1], viewer);
		...
		TableViewerColumn downloadButtonColumn = createTableViewerColumn(titles[2], bounds[2], viewer);
	...
	public void controllerUpdateNotify(ControllerUpdateType updateType, Object updatedEntity) {
		if (updatedEntity instanceof UserFile && ((UserFile)updatedEntity).getSubTask().getId()==taskSubTaskInfoLeft.getSubTask().getId()) {
